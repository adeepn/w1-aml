...
static struct ieee80211_channel aml_5ghz_a_channels[] =
{
...
static struct ieee80211_rate aml_rates[] =
{
...
{
    memcpy((void*)channels, (void*)aml_5ghz_a_channels,
...
#endif
    //printk("%s timestamp:%lld %s, rssi:%d\n", __func__, timestamp, lse->SI_ssid + 2, rssi);
...
void vm_cfg80211_indicate_connect(struct wlan_net_vif *wnet_vif)
{
    ...
    const unsigned char *req_ie = NULL;
    size_t req_ie_len = 0;
    ...
    {
        DPRINTF(AML_DEBUG_CFG80211, "%s %d wnet_vif->vm_opt_ie_len=%d add req_ie_len\n",
            __func__, __LINE__, wnet_vif->vm_opt_ie_len);
        req_ie = wnet_vif->vm_opt_ie;
        req_ie_len = wnet_vif->vm_opt_ie_len;
    }
    ...
    if (bss != NULL) {
        cfg80211_connect_bss(wnet_vif->vm_ndev, sta->sta_bssid, bss, req_ie, req_ie_len, wnet_vif->assocrsp_ie.ie,
...
#else
    cfg80211_connect_result(wnet_vif->vm_ndev, sta->sta_bssid, req_ie, req_ie_len, wnet_vif->assocrsp_ie.ie,
        wnet_vif->assocrsp_ie.length, WLAN_STATUS_SUCCESS, GFP_ATOMIC);
#endif
...
vm_cfg80211_scan(struct wiphy *wiphy, struct cfg80211_scan_request *request)
{
    ...
    /*need thinking more, maybe there are bugs hapen*/
        ...
        switch (lsme->auth_type) {
            ...
            case NL80211_AUTHTYPE_AUTOMATIC:
                if (lsme->crypto.n_ciphers_pairwise && lsme->key_len)
                    sta_authmode = WIFINET_AUTH_AUTO;
                else
                    sta_authmode = WIFINET_AUTH_OPEN;
                break;
    ...
    struct net_device *dev, unsigned short reason_code)
{
    ...
    switch (wnet_vif->vm_opmode)
    {
        case WIFINET_M_STA:
            ...
            wifi_mac_top_sm(wnet_vif, WIFINET_S_INIT, reason_code);
            if (wnet_vif->vm_p2p_support) {
                wifi_mac_top_sm(wnet_vif, WIFINET_S_SCAN, 0);
            }
            break;
...
int wifi_mac_save_app_ie(struct wifi_mac_app_ie_t  * app_ie,
                        const unsigned char *app_buf,int app_buflen)
{
    void * ie;
    void * tmpPtr;
